...
class Futures {
  ...
  def insert[T](doc: T)(implicit name: String, executor: ExecutionContext): Future[Result] = {
    ...
    Future {
      ...
      case _ => promise.success(Result(true, 421))
    }
...
case class Result(success: Boolean = true, status: Int = 200, timestamp: Long = System.currentTimeMillis)

case class EitherMappableFuture[A](future: Future[A]) {
  def mapEither[E <: Throwable, B](f: A => Either[E, B])(implicit ec: ExecutionContext): Future[B] = {
    future.flatMap(
      f(_) match {
